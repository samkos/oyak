...
function affiche(reponse) {
   ...
   while (blocs.length>1) {
	   ...
	   break;
       case 'data' :
	   process_data_into_graphics(content);
	   break;
       case 'message' :
...
function process_data_into_graphics(content) {
    var data1 = [];    
    var data2 = [];    
	...
	while (chunks.length>0) {
	    ...
	    a = chunk.split(":");
	    //alert(a[0]+"="+a[1]);
	    data1.push([parseInt(a[0]), parseInt(Math.floor(a[1]))]);
	    data2[nb_data] = { label: a[0], data: parseInt(Math.floor(a[1]))};
	    nb_data = nb_data+1;
	}
    var stack = 0, bars = true, lines = false, steps = false;
    $("div#graphique").replaceWith(" <div  id='graphique' > \
                                     <div  id='graphique1'  \
                                      style='padding:10px; width:490px;height:300px; '  \
                                      class='graph' > \
                                           </div>      \
                                     <div  id='graphique2'  \
                                      style='padding:10px; width:490px;height:300px; '  \
...
function create_file_dir() {
    $("div#graphique").replaceWith(" <div  id='graphique'  \
                                      style='padding:10px; background:#D0DCE0;width:900px;height:300px; '  \
                                      class='graph' > \
                                        <a href='../' \
                                           title='Free Web File Manager'>Back to Free Web File Manager</a> \
	                                   <div style='height: 16px; line-height: 16px'>&nbsp;</div> \
                                           <div id='fileTreeDemo_1' class='demoto'> \
                                           </div>      \
                                     </div>");
        
    jQuery('#fileTreeDemo_1').gsFileManager({ script: 'http://localhost:5555/GsFileManager.php' });
}


function create_filetree_dir() {
    $("div#graphique").replaceWith(" <div  id='graphique'  \
                                      style='padding:10px; width:900px;height:300px; '  \
                                      class='graph' > \
                                           <div id='fileTreeDemo_1' class='demo'> \
                                           </div>      \
                                     </div>");
        
    $('#fileTreeDemo_1').fileTree({ root: '.', script: 'jqueryFileTree.py' }, function(file) { 
	    alert(file);
	});
}
